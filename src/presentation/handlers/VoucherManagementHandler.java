...
public class VoucherManagementHandler {
    ...
    public void handleVoucherManagement() {
        while (true) {
            ...
            ConsoleUtils.printCenter(ConsoleUtils.CYAN + "━━━━━━━━ ＶＯＵＣＨＥＲ ＭＡＮＡＧＥＭＥＮＴ ━━━━━━━━" + ConsoleUtils.RESET,
    ...
    private void viewAllVouchers() {
        try {
            ...
            ConsoleUtils.printLogoCentered(logo, ConsoleUtils.DEFAULT_WIDTH, 88);
            ConsoleUtils.printCenter(
                    ConsoleUtils.GREEN + "━━━━━━━━━━━━━ ＡＬＬ ＶＯＵＣＨＥＲＳ ━━━━━━━━━━━━━" + ConsoleUtils.RESET,
                    ConsoleUtils.DEFAULT_WIDTH);
            System.out.println();

            List<VoucherCode> vouchers = voucherDAO.findAll();
            if (vouchers == null || vouchers.isEmpty()) {
                ConsoleUtils.printCenter(ConsoleUtils.YELLOW + "No vouchers available!" + ConsoleUtils.RESET,
                        ConsoleUtils.DEFAULT_WIDTH);
            } else {
                String[] headers = { "ID", "Code", "Discount", "Max Uses", "Used", "Remaining", "Status", "Expires" };
                ...
                for (int i = 0; i < vouchers.size(); i++) {
                    ...
                    String maxUsesStr = voucher.isUnlimited() ? "Unlimited" : String.valueOf(voucher.getMaxUses());
                    String remainingStr = voucher.getRemainingUsesDisplay();

                    // Get short status (without description)
                    String status = voucher.getStatus() != null ? voucher.getStatus().name() : "INACTIVE";

                    String expiresStr = voucher.getExpiresAt() != null ? voucher.getExpiresAt().toLocalDate().toString()
                            : "N/A";
    ...
    private void createVoucher() {
        try {
            ...
            ConsoleUtils.printLogoCentered(logo, ConsoleUtils.DEFAULT_WIDTH, 88);
            ConsoleUtils.printCenter(ConsoleUtils.GREEN + "━━━━━━━━ ＣＲＥＡＴＥ ＮＥＷ ＶＯＵＣＨＥＲ ━━━━━━━━" + ConsoleUtils.RESET,
                    ConsoleUtils.DEFAULT_WIDTH);
            System.out.println();
            ...
            if (code.isEmpty()) {
                ConsoleUtils.printCenter(ConsoleUtils.RED + "Voucher code cannot be empty!" + ConsoleUtils.RESET,
                        ConsoleUtils.DEFAULT_WIDTH);
                System.out.print(centerText(ConsoleUtils.YELLOW + "Press Enter to continue..." + ConsoleUtils.RESET,
                        ConsoleUtils.DEFAULT_WIDTH));
                sc.nextLine();
                return;
            }
            ...
            if (discountType == enums.DiscountType.PERCENT
                ...
                ConsoleUtils.printCenter(
                        ConsoleUtils.RED + "Percentage discount cannot exceed 100%!" + ConsoleUtils.RESET,
                        ConsoleUtils.DEFAULT_WIDTH);
                System.out.print(centerText(ConsoleUtils.YELLOW + "Press Enter to continue..." + ConsoleUtils.RESET,
                        ConsoleUtils.DEFAULT_WIDTH));
                sc.nextLine();
            ...
            // Input max uses
            System.out.print(centerText(ConsoleUtils.CYAN + "Enter max uses (0 = unlimited): " + ConsoleUtils.RESET,
                    ConsoleUtils.DEFAULT_WIDTH));
            int maxUses = Integer.parseInt(sc.nextLine().trim());

            // Input description
            System.out.print(centerText(
                    ConsoleUtils.CYAN + "Enter description (optional, press Enter to skip): " + ConsoleUtils.RESET,
                    ConsoleUtils.DEFAULT_WIDTH));
            String description = sc.nextLine().trim();

            // Input expiry date
            System.out.print(
                    centerText(ConsoleUtils.CYAN + "Enter expiry date (yyyy-MM-dd, or press Enter for no expiry): "
            ...
            if (!expiryInput.isEmpty()) {
                try {
                    ...
                    ConsoleUtils.printCenter(ConsoleUtils.RED + "Invalid date format!" + ConsoleUtils.RESET,
                            ConsoleUtils.DEFAULT_WIDTH);
                    System.out.print(centerText(ConsoleUtils.YELLOW + "Press Enter to continue..." + ConsoleUtils.RESET,
                            ConsoleUtils.DEFAULT_WIDTH));
                    sc.nextLine();
            ...
            System.out.println();
            ConsoleUtils.printCenter(ConsoleUtils.GREEN + "✓ Voucher created successfully!" + ConsoleUtils.RESET,
                    ConsoleUtils.DEFAULT_WIDTH);
            System.out.println();
            System.out.print(centerText(ConsoleUtils.YELLOW + "Press Enter to continue..." + ConsoleUtils.RESET,
                    ConsoleUtils.DEFAULT_WIDTH));
    ...
    private void updateVoucher() {
        try {
            ...
            ConsoleUtils.printLogoCentered(logo, ConsoleUtils.DEFAULT_WIDTH, 88);
            ConsoleUtils.printCenter(ConsoleUtils.YELLOW + "━━━━━━━━━ ＵＰＤＡＴＥ ＶＯＵＣＨＥＲ ━━━━━━━━━" + ConsoleUtils.RESET,
                    ConsoleUtils.DEFAULT_WIDTH);
            System.out.println();
            ...
            // Display current voucher details in table format
            System.out.println();
            String[] headers = { "Field", "Current Value" };
            int[] columnWidths = { 20, 40 };
            ConsoleUtils.printTableHeader(headers, columnWidths, 140);

            String[] fields = { "Code", "Discount Type", "Discount Value", "Max Uses", "Used Count", "Status",
                    "Expires At" };
            String[] values = {
                    voucher.getCode(),
                    voucher.getDiscountType() != null ? voucher.getDiscountType().getDescription() : "N/A",
                    voucher.getDiscountValue() != null ? voucher.getDiscountValue().toString() : "N/A",
                    ...
                    voucher.getExpiresAt() != null ? voucher.getExpiresAt().toLocalDate().toString() : "No expiry"
            };
            ...
            String[] updateColors = { ConsoleUtils.GREEN, ConsoleUtils.GREEN, ConsoleUtils.GREEN, ConsoleUtils.GREEN,
                    ConsoleUtils.RED };
            ...
            switch (updateChoice) {
                case 1:
                    System.out.print(centerText(ConsoleUtils.CYAN + "Enter new discount value: " + ConsoleUtils.RESET,
                            ConsoleUtils.DEFAULT_WIDTH));
                ...
                case 2:
                    System.out.print(
                            centerText(ConsoleUtils.CYAN + "Enter new max uses (0 = unlimited): " + ConsoleUtils.RESET,
                                    ConsoleUtils.DEFAULT_WIDTH));
                ...
                case 3:
                    System.out.println();
                    ConsoleUtils.printCenter(ConsoleUtils.CYAN + "Select new status:" + ConsoleUtils.RESET,
                            ConsoleUtils.DEFAULT_WIDTH);
                    System.out.println();
                    String[] statusOptions = { "1. Active", "2. Inactive", "3. Expired" };
                    String[] statusColors = { ConsoleUtils.GREEN, ConsoleUtils.ORANGE, ConsoleUtils.RED };
                    ConsoleUtils.printMenuOptions(statusOptions, ConsoleUtils.DEFAULT_WIDTH, statusColors);
                    System.out.println();
                    System.out.print(centerText(ConsoleUtils.YELLOW + "Choose (1-3): " + ConsoleUtils.RESET,
                            ConsoleUtils.DEFAULT_WIDTH));
                    ...
                    if (statusChoice == 1) {
                        ...
                        ConsoleUtils.printCenter(ConsoleUtils.RED + "Invalid choice!" + ConsoleUtils.RESET,
                                ConsoleUtils.DEFAULT_WIDTH);
                        System.out.print(
                                centerText(ConsoleUtils.YELLOW + "Press Enter to continue..." + ConsoleUtils.RESET,
                                        ConsoleUtils.DEFAULT_WIDTH));
                        sc.nextLine();
                        return;
                    }
                ...
                case 4:
                    System.out.print(centerText(ConsoleUtils.CYAN
                            + "Enter new expiry date (yyyy-MM-dd, or press Enter for no expiry): " + ConsoleUtils.RESET,
                            ConsoleUtils.DEFAULT_WIDTH));
                    ...
                    if (newExpiryInput.isEmpty()) {
                        ...
                        try {
                            java.time.LocalDate newExpiryDate = java.time.LocalDate.parse(newExpiryInput);
            ...
            System.out.println();
            ConsoleUtils.printCenter(ConsoleUtils.GREEN + "✓ Voucher updated successfully!" + ConsoleUtils.RESET,
                    ConsoleUtils.DEFAULT_WIDTH);
            System.out.println();
            System.out.print(centerText(ConsoleUtils.YELLOW + "Press Enter to continue..." + ConsoleUtils.RESET,
                    ConsoleUtils.DEFAULT_WIDTH));
    ...
    private void deactivateVoucher() {
        try {
            ...
            ConsoleUtils.printLogoCentered(logo, ConsoleUtils.DEFAULT_WIDTH, 88);
            ConsoleUtils.printCenter(ConsoleUtils.ORANGE + "━━━━━━━━ ＤＥＡＣＴＩＶＡＴＥ ＶＯＵＣＨＥＲ ━━━━━━━━" + ConsoleUtils.RESET,
                    ConsoleUtils.DEFAULT_WIDTH);
            System.out.println();

            // Get voucher ID
            System.out.print(centerText(ConsoleUtils.CYAN + "Enter voucher ID to deactivate: " + ConsoleUtils.RESET,
                    ConsoleUtils.DEFAULT_WIDTH));
            int voucherId = Integer.parseInt(sc.nextLine().trim());
            ...
            ConsoleUtils.printTableRow(new String[] { voucher.getCode(), discount, status }, columnWidths, 140);
            ConsoleUtils.printTableFooter(columnWidths, 140);
            ...
            if (confirmation.equals("yes") || confirmation.equals("y")) {
                ...
                System.out.println();
                ConsoleUtils.printCenter(
                        ConsoleUtils.GREEN + "✓ Voucher deactivated successfully!" + ConsoleUtils.RESET,
                        ConsoleUtils.DEFAULT_WIDTH);
            } else {
                System.out.println();
                ConsoleUtils.printCenter(ConsoleUtils.YELLOW + "Operation cancelled!" + ConsoleUtils.RESET,
                        ConsoleUtils.DEFAULT_WIDTH);
            }
